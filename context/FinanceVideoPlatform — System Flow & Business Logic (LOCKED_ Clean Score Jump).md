According to a document from **January 17, 2026** (la Biblia **LOCKED — Clean Score Jump**) y el flow que mandó Royki, aquí tienes el **flow reescrito** (en Markdown) ya corregido: sin “assets pack”, con **intención del clip definida desde el inicio**, con **QA enforcement**, con **Veo 3.1 \= 8s fijo**, y con **montaje consecutivo usando solo \~4s útiles por clip** (el descalce con VO se resuelve en edición posterior).
...
* `node_role`: subtipo interno (ampliable sin romper node\_type\_base)  
...
* `clips/*.mp4`  
* `qc_report.json` (incluye intentos, fails, chosen edit\_window)  
* `_CLIPS_DONE.json`
...
| ----- | ----- | ----- | ----- |
| 0\. Ingest & QC | Script \+ Audio \+ Biblia LOCKED | Validación \+ (opcional) vo\_map | `run_manifest.json`, `vo_map.json` |
...
| 4\. Clips | Init Frames \+ Clip prompts | Veo 3.1 (8s) \+ QA \+ edit\_window | `clips/*.mp4`, `qc_report.json` |
...
* No assets pack; sí caché por beat\_id y reuse cross-beat/cross-episodio con firma \+ compatibility check.

---

### **Notas (puedes borrar antes de enviar)**

* El flow original de Royki (“style\_bible opcional”, Visual Director creativo, duración por forced alignment, assembly “sincronizado”) está en `flow.md`.  
* Los hard-locks, two-pass prompting, QA gates, shot menu y los 3 inputs obligatorios salen de la Biblia LOCKED 01172026\.
